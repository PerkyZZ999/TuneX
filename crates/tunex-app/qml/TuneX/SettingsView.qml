...
Item {
    ...
    readonly property list<string> shortcutActions: [qsTr("Play / pause"), qsTr("Play, next, previous"), qsTr("Volume and mute"), qsTr("Search"), qsTr("Clear selection, then close Now Playing, then search, then back"), qsTr("Select all loaded rows"), qsTr("Extend the track selection"), qsTr("Back / forward"), qsTr("Move list and grid cursor"), qsTr("Move list and grid cursor"), qsTr("Play the current track, or open the current album or artist"), qsTr("Jump to the first title with that prefix (a leading j/k still moves; 400 ms reset)"), qsTr("Cycle Tracks, Albums, and Artists result groups")]
