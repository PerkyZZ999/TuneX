...
- **Waiver:** none
- **Follow-up:** W-074 positional drops + playlist-add toast (entry below); AUR publish stays a human confirmation.

### 2026-09-15 — W-074 positional drops + playlist-add toast
- **Slice:** post-S16 polish (no slice reopened)
- **Result:** pass
- **Evidence:** One drop rule everywhere: drops land where the feedback showed. `TrackDropArea` gains a positional mode (`targetView`): the hover computes the insertion index from the drag position and draws the accent line plus a `#N of M` caption (never colour-only); plain mode (playlist sidebar rows) keeps the wash and appends. Drags carry `application/x-tunex-origin` (empty external, `queue`, `playlist:<id>`) plus source rows, so an internal drag moves rows while an external one inserts: new `Queue::insert_at` / `Queue::move_items` (cursor + history track the same tracks), `PlaybackController` pass-throughs, `QueueModel.enqueueTrackIdsAt` / `moveItems`, `playlist::{insert_into_playlist, move_entries}`, `PlaylistModel.addTracksAt`, `PlaylistTrackModel.insertTracks` / `moveItems` — all clamped, order-preserving, moves never copy. The drag chip rides up-right of the cursor so the insertion line stays visible. Every playlist add (row-menu Add to playlist incl. New playlist, SelectionBar New playlist, sidebar drops) closes the menu and shows the shell `Toast` (DESIGN.md toast: surface-raised, icon + one-line copy, 5 s, polite announcement) naming the count and the list; failures toast the error text. Detail/queue drops land visibly and surface failures on the views' error lines instead. Tests: queue insert/move (incl. cursor tracking), playlist insert/block-move, bridge positional round-trips — 327 green. `scripts/qml-lint.sh` 48/48 ✓. `rust-tc doctor` ✓ (327 tests). CMake configure/build ✓; offscreen boot loads the shell with no QML errors. Traceability: R-010 + R-012 → W-074 → this entry.
- **Waiver:** none
- **Follow-up:** W-075 (entry below); GUI visual proof deferred per maintainer request.

### 2026-09-15 — W-075 drag auto-scroll + landing cursor
- **Slice:** post-S16 polish (no slice reopened)
- **Result:** pass
- **Evidence:** `TrackDropArea` positional mode auto-scrolls `targetView.contentY` while a drag hovers in the 48px top/bottom edge zones (8–28px per 32ms tick, faster deeper in the zone; the insertion line recomputes after each step), so long lists can receive positional drops; functional scrolling stays on with reduce-motion. After a drop the keyboard cursor (`currentIndex`) lands on the inserted/moved rows in both Now Playing and playlist detail. Empty lists caption the line `Drop here` instead of `at the end`. GUI proof deferred: kwin-mcp isolated sessions verified working in this environment (session start, EIS input backend, full TuneX a11y tree with coordinates) but the maintainer stopped the GUI pass before the flow ran; auto-scroll + cursor logic is covered by the W-074 unit tests plus qmllint typing. `scripts/qml-lint.sh` 48/48 ✓. CMake build ✓; offscreen boot clean. Traceability: R-010 + R-012 → W-075 → this entry.
- **Waiver:** none
- **Follow-up:** Phase 7 Release (AUR publish) stays a human confirmation.
