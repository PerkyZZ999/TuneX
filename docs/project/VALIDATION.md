...
### 2026-09-15 — UI/UX polish pass F1–F10 (pass three, owner-requested)
- **Slice:** none (outside WORK_ITEMS; no locked decision touched, no new screens or models)
- **Result:** pass (10 commits, each: `scripts/qml-lint.sh` clean + CMake build green + hook checks green)
- **Evidence:** F6 put the volume tooltip on the overlay slider (last one missing it). F5 gives the lyrics toggle the note glyph — it shared list-music with the queue toggle. F8 documents the leading-j/k exception in type-to-select. F10 pairs the rail error line with the alert glyph like the overlay. F9 offers Clear search on per-tab no-match states (drill empty excluded). F7 scrolls the more-by-artist rail to the focused card (E11 extension). F1/F2 constrain numeric fields to digits at the source (Days 1–36500, track/disc 1–9999, year 0–9999; empty still means unchanged). F3 keeps rejected playlist names in the dialog with an inline error instead of closing and losing the input. F4 glides synced lyrics to the sung line on changes and on open. Detour worth noting: an inline `property Validator` type does not resolve (needs an unseen QtQml import), so per-instance IntValidators are used; a stray duplicated handler line from editing was caught by qmlformat failing to parse and repaired. Media-test flakes under parallel load persist, green solo.
- **Waiver:** none
- **Follow-up:** Phase 7 Release (AUR publish) stays a human confirmation.

### 2026-09-15 — Architecture pass H1–H5 (whole codebreadth, owner-requested)
- **Slice:** none (outside WORK_ITEMS; D-008 crate seams untouched — file-internal and QML-module deepening only)
- **Result:** pass (5 commits, each: gates green — `rust-tc doctor` for Rust, `scripts/qml-lint.sh` + CMake for QML)
- **Evidence:** H4 shares one `friendly_error` on the bridge root (was byte-identical in both playlist modules). H5 repaints the waveform Canvas only while shown (CSV ticks at the frame cap in every mode). H1 adds a TuneX.Format singleton — monogram x8, duration x5, singular/plural x9 behind three pure functions; closes the C5 shared-helper deferral properly (a .js library cannot join the cxx-qt module, a registered singleton can). H2 adds TuneX.Field — five styled text fields (two playlist dialogs, eight tag fields, profile field) behind one chrome module. H3 adds SettingsCycleRow — repeat/ReplayGain/output/view/preset rows behind title/value/icon/names/checkable props; also drops the Controls import H2 orphaned in three dialogs. Assessed and deliberately skipped: per-tab TrackListView / sort menus / Home rails (each site's wiring genuinely differs — a shared module would be shallow, failing the depth test), PlayerState sync singleton (blast radius across four surfaces with no GUI proof available), queue_model/db.rs splits (big but coherent — cosmetic), Shape waveform rewrite (needs profiler evidence per M6 discipline; the repaint guard is the safe win). Mid-pass repair: a bad H4 edit briefly dropped an impl block — caught by immediate diff review, restored, verified by check.
- **Waiver:** none
- **Follow-up:** Phase 7 Release (AUR publish) stays a human confirmation.

### 2026-09-15 — Architecture pass K1–K12 + E12 revert (whole codebase, owner-requested, Kwin-proven)
- **Slice:** none (outside WORK_ITEMS; D-008 crate seams untouched)
- **Result:** pass (13 commits, each: gates green — `rust-tc doctor` for Rust, `scripts/qml-lint.sh` + CMake for QML)
- **Evidence:** K1 selection.mimeIds(model) kills seven mime builders + dead ternary + stamp dance. K2 SettingsSection wraps five pages (spacing-identical). K3 HoverPlay unites card veils + badges. K4 Format.fallback/durationOrDash own eleven fallbacks + three dashes. K5/K6 smart + home chips become Repeaters. K7 SearchingIndicator unites visible/running. K8 composes accessible names with placeholders (TrackRow keeps conditional suffixes). K9 opens row menus from Menu/Shift+F10 on queue/facet/album/artist lists and drops the never-firing row handlers + dead focus branch. K10 ArtPump drains all four cover resolvers. K11 repeat labels move to Format. K12 BrowseRow unites folder/facet delegates (also dropping their dead focus terms). Plus a revert: E12 wrapped tabLabel — which already carried counts — in a second count; chips read Tracks (12) (12). Reverted to the single source of truth.
- **Live proof (isolated Kwin, isolate_home, tagged 6-track fixtures):** search tabs show single counts (e12r-search-tabs.png); queue row menu opens from the Menu key (k9-queue-row-menu.png); selection count + queue flow work (k1-search-selection.png); Home chips/mono rails (k-home-populated.png); Tracks incl. Unknown Artist + Title-arrow sort + Play-all count (k-tracks.png); Folders BrowseRow (k-folders.png); hover PlayBadge + monogram grid (k-albums-hover.png); smart sidebar badge (k-playlists-smart.png); smart rule line + evaluated Play-all count (k-smart-rule.png); tag editor with all eight Fields (k-tagedit.png); duplicate-name dialog stays open with inline error (k-f3-duplicate-kept.png); E5 track title in the window bar (seen live); K8 names observed in the AT-SPI tree. Fixture quarantine: my own sloppy DB surgery inserted a phantom smart rule onto Evening — the E6 badge correctly flagged it, phantom deleted, sidebar re-verified.
- **Proven-by-construction (noted, not shot):** K2/H3 wrappers (spacing-identical, lint-clean), K5/K6/K7 Repeaters and spinner (structural), K11/K4 strings (byte-identical output), C3/E1 dialogs (same GlassDialog mechanics as F3-proven), F1 Days typing + F4 follow + lyrics toggle + volume tooltip + E11 rails (documented gaps for a focused pass). Two live attempts failed for harness reasons, both recorded: EIS clicks fire TrackRow onReleased but not IconButton onClicked (W-002 class); AT-SPI focused state does not always equal QML activeFocus (verify rings visually). A suspected Space double-toggle investigated end-to-end (rail vs Play, screenshots + tree) resolved to no bug: the global shortcut and item handlers coordinate correctly. Validator screenshot lost to a session_stop cleanup race (copied in parallel with stop) — observed and recorded anyway.
- **Waiver:** none
- **Follow-up:** Phase 7 Release (AUR publish) stays a human confirmation.

### 2026-09-15 — Deepening pass post-K1-K12 (13 commits, owner-requested)
- **Slice:** none (outside WORK_ITEMS; D-008 crate seams untouched — shared helpers and dead-code removal only, plus one a11y fix)
- **Result:** pass (gates re-verified headless over the full 13-commit range: `scripts/qml-lint.sh` 58/58 clean, CMake configure+build green, `rust-tc quick` 319/319, `rust-tc doctor` PASS)
- **Evidence:** Library Play-all/facet concatenation collapses six hand-rolled group loops plus two enqueue sweeps onto one `concat_group_tracks` helper, with a new genres-facet concatenation test. Search gains `SearchCore::poll_mapped` so settled results drain through one poll-then-map seam. Bridge `Debug` for the eight generated roles enums comes from one `debug_roles!` macro (derive is not permitted on `#[qenum]`). QML: one `DrillHeader` backs album/folder/result drill views; one `CapFooter` backs capped result groups; every track list opens its row menu through `TrackMenu.openFor` (bounds-check + cursor seed + selection ids); `TrackListSelection` drops the `stamp` version counter (every mutation reassigns `rows`, so host bindings stay subscribed) and gains `removeSelected` (bottom-up `removeAt`); grid cell math, transport icon choice, and card meta lines move into `Format`; Play-all kind dispatch collapses onto the tab key with one tab→sort-args helper. The one behavior change is a11y: search result grids are keyboard-operable — j/k move, Return/Enter opens (album/artist/songs), Tab still cycles Songs → Albums → Artists groups by design, mirroring the library grids.
- **Waiver:** none (no live KWin proof this pass — behavior-preserving refactors verified by lint/build/unit; the search-grid keyboard map is new UI behavior and wants a focused keyboard walkthrough next GUI session)
- **Follow-up:** Phase 7 Release (AUR publish) stays a human confirmation.

### 2026-09-15 — Search-grid keyboard walkthrough (2628562 GUI proof)
- **Slice:** none (no code change — live proof for the post-K1-K12 a11y fix)
- **Result:** pass with two pre-existing follow-ups below (no regression from the tested commit)
- **Evidence:** Isolated KWin at 1600×1000 (`isolate_home`, scratch XDG with a seeded `config.toml` root, never `~/Music`); 6-track tagged library (`mid3v2` ID3 on sine.mp3 copies: Harbor Nova ×4 + Pale Meridian ×2 across Harbor Lights / Cinder Harbor / Glass Tide); query `harbor` settles to Tracks (5) / Albums (3) / Artists (2). **Songs:** Down focuses "Song results", j/k move the highlight (Harbor Ash → Harbor Tide screenshots), Return plays the focused row (window title `Harbor Ash — Harbor Nova · TuneX`, Now Playing (1), pause transport). **Albums:** Tab cycles songs→albums with grid focus, j moves Cinder Harbor → Harbor Lights, Return drills into the Harbor Lights detail (Play/Queue + tracks + More-by rail). **Artists:** Tab cycles albums→artists with grid focus, j moves Harbor Nova → Pale Meridian, Return opens the Pale Meridian detail (header Play + album + tracks). Forward Tab-cycling proven from both the songs list and the search field; query + tab survive drill + two history Backs. Evidence `docs/project/evidence/kb-*.png` (13). App log clean of flow errors (boot-only GlassDialog binding-loop + portal + AT-SPI adaptor noise, all pre-interaction).
- **Waiver:** none
- **Follow-up:** both gaps below are now FIXED — see the next entry (c275ddf, d41f2e8). Harness notes: `frame_*.png` names collide per session (copy out immediately, `cmp`-verify before stopping); never batch dependent input + file-copy calls in one block (they can run concurrently).

### 2026-09-15 — Search Tab-cycling fixes (Backtab + card tab stops, owner-requested)
- **Slice:** none (outside WORK_ITEMS; search keyboard behavior only, no locked decision touched)
- **Result:** pass (2 commits, each: `scripts/qml-lint.sh` clean + CMake build green + hook `doctor` green; live KWin proof below)
- **Evidence:** (1) `c275ddf` accepts `Qt.Key_Backtab` alongside `Qt.Key_Tab` in all four group-cycling branches (App.qml search field + SearchView songs/albums/artists grids) and treats Backtab as backward unconditionally. (2) `d41f2e8` adds a `tabFocusable` flag (default `true`) to AlbumCard/ArtistCard bound to `activeFocusOnTab`, with only the two SearchView delegates opting out — library, home, and detail views keep their tab stops unchanged. Cards keep their Accessible Button roles and grid j/k + Return operation, mirroring the song rows.
- **Live proof (isolated KWin, same seeded library/query as the walkthrough):** held-Shift+Tab from the albums grid cycles back to Songs (`fx-shift-tab-back.png`); held-Shift+Tab from the search field cycles artists→albums (`fx-field-shift-tab.png`); plain Tab after j-highlighting an album card cycles albums→artists instead of escaping to chrome (`fx-tab-from-card.png`); Return still drills into the album detail with the new delegates (`fx-return-drill.png`). Forward cycling re-verified intact. App log clean of flow errors. AT-SPI bus dropped mid-session (`at-spi-bus-launcher` registration failure) — verification continued on screenshots + window titles per the skill's fallback rule; the bus recovered nothing, nothing depended on it.
- **Waiver:** none
- **Follow-up:** Phase 7 Release (AUR publish) stays a human confirmation. Library/home card tab stops deliberately unchanged (proven by construction: default-`true` binding evaluates identically; no live pass over those views this session).
