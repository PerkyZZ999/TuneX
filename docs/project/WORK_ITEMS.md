...
- [ ] W-005 — In-memory queue + about-to-finish preload + next/prev/shuffle/repeat (R-010) — BLOCKED 2026-09-09: `gst-plugins-good` (wavparse) missing on this machine, no passwordless sudo. Queue pure logic done + tested; gapless test written, fails loud with “missing a plug-in”. Unblocks with `sudo pacman -S --needed gst-plugins-good`.
