...
- **Rhythm:** 4px base unit; section gaps 20–28px (`lg`–`xl`); card internal padding 8–12px; list rows 32px (same compact chrome as the labeled rail) with a 2px hairline gap so hover/selected chips do not fuse; 50k lists stay scannable. Page gutters 20px, never less than 16px.
...
- **Track row (`track-row` → `track-row-hover` / `selected`):** 32px (same density as labeled nav), transparent over the canvas, 2px gap between rows; columns number · title/body-md + artist/body-sm-muted stacked at line-height 1.0 (no extra gap between the two lines) · 32px more action · duration/caption-muted right. Now Playing rail/drawer drops the leading index (the em dash on unnumbered queue rows) and the ⋯ — right-click / Menu / Shift+F10 still open the row menu — and steps title to body-sm and artist to caption so the 288px panel can show more of the name. Playing row: accent 3px bar flush to the leading edge + title in foreground (not accent-colored text) + animated equalizer tick allowed only with motion on. Selected rows fill `{colors.selected}` on the row itself — that fill is not the playing marker; a playing selected row shows both. ListView highlight stays the keyboard cursor only. Missing-file rows dim to 50% with "File missing" caption + actions. Folder, genre, composer, playlist-sidebar, and Now Playing meta share this tight stack. Drag: the source row dims to 50%; a surface-raised chip (1px accent edge, title or “N songs”) follows the pointer. Drop targets wash `{colors.selected}` with the same accent edge plus a caption (“Drop in Now Playing” / “Drop in playlist”) — never colour-only, never glow. Positional targets (Now Playing, playlist detail) instead draw the accent insertion line at the landing boundary with a `#N of M` caption; the chip rides clear of the pointer so the line stays visible.
